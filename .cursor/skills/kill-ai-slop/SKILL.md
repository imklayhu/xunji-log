...
> **Murmur 完整流程**见 [`../murmur-design-sop/SKILL.md`](../murmur-design-sop/SKILL.md)（产品→设计拨盘→交互文案→动效→物料→去 slop→验收）。
...
修 slop 时若命中「emoji 当图标 / 假纹理 / 缺品牌位 / 外链随机图」，在本仓库用物料服务替换，而不是另画一套：

```bash
cd services/materials && uv run murmur-assets derive
# 字标 → apps/desktop/public/murmur.svg
# 纸纹 → apps/desktop/public/paper-noise-texture.svg（极低 opacity）
# 图标 → apps/desktop/src-tauri/icons/
```

完整决策表：`.cursor/skills/materials-in-design/references/workflow.md`。不要引入吉祥物或装饰插画来「填空」。
...
- `.cursor/skills/materials-in-design/` — Murmur 物料服务衔接（字标/纸纹/图标）。
