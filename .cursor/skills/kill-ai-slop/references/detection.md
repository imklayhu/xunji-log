...
Hard to grep purely; look for a serif font family or ` /italic` applied
...
```
*Confirm:* the italic/serif span sits within an ` / ` whose base font is
sans.
...
```
*Confirm:* an inline ` ` that's a blob-with-dot-eyes or a cartoon creature
